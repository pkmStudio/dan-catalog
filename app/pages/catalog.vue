...
const { data, status, error, refresh } =
  await useFetch<ApiResponse<ProductGroup[]>>('/api/catalog/groups')
...
<template>
  <main>
    <PageHero
      ...
      subtitle="Выберите группу товаров, чтобы перейти к подходящим запчастям"
      :crumbs="[{ label: 'Каталог' }]"
    />
    <section class="content-section">
      <div class="section-heading">
        <h2>Группы запчастей</h2>
        <span>{{ groups.length }} категорий</span>
      </div>
