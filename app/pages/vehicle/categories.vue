...
<template>
  <main v-if="vehicle">
    <PageHero :title="`Категории для ${vehicle.makeName} ${vehicle.modelName}`"
    ...
    <section class="content-section">
      <div class="selected-vehicle"><span><b>Выбранный автомобиль</b><small>{{ vehicle.makeName }} {{
          vehicle.modelName
        }} · {{ vehicle.modification }}</small></span>
        <NuxtLink to="/vehicle">Изменить</NuxtLink>
      </div>
      <div class="section-heading"><h2>Группы запчастей</h2><span>16 категорий для выбранного автомобиля</span></div>
